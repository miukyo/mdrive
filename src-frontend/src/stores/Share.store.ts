...
export const useShareStore = create<Store>()((set) => ({
	createShare: async (type: "file" | "folder", folderId?: number, messageId?: number) => {
		...
		if (!response.ok) return { success: false, message: data.error?.message || "Failed to create share" };
	...
	getShares: async () => {
		...
		const data = await response.json();
		if (!response.ok) return { success: false, message: data.error?.message || "Failed to fetch shares" };

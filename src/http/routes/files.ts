...
export const filesRouter = new Elysia({ prefix: "/files" })
  ...
  .get(
    "/",
    async ({ query: { folder_id }, sessionId }) => {
    ...
    {
      query: t.Object({
        folder_id: t.Optional(
          t.String({ description: "Optional folder ID to list files from" }),
        ),
      }),
      detail: {
        summary: "List files",
  ...
  .post(
    "/upload",
    async ({
      body: { files: uploadedFiles, folder_id, transfer_ids, transfer_id },
      sessionId,
    }) => {
      ...
      for (const fileObj of files) {
        ...
        const tempPath = path.join(
          UPLOAD_TEMP_DIR,
          `${crypto.randomUUID()}_${originalname}`,
        );
        ...
        for (let i = 0; i < totalChunks; i++) {
          ...
          const uploadedFile = await client.uploadFile({
            ...
            onProgress: (progress) => {
              const chunkPercent =
                (progress as number) * (currentChunkSize / size) * 100;
          ...
          if (result instanceof Api.Message) {
            ...
            const peerId =
              resolvedPeer instanceof Api.User ||
              resolvedPeer instanceof Api.Channel ||
              resolvedPeer instanceof Api.Chat
                ? Number(resolvedPeer.id)
                : 0;
  ...
  .post(
    "/delete",
    async ({ body: { message_ids }, sessionId }) => {
      const session = await getSession(sessionId!);
      if (!session) throw new ApiError(401, "Session not found");
      ...
      const allMessageIds = await getAllChunkMessageIds(
        session.phone,
        numericIds,
      );
  ...
  .post(
    "/move",
    async ({ body: { message_ids, to_folder_id }, sessionId }) => {
      const session = await getSession(sessionId!);
      if (!session) throw new ApiError(401, "Session not found");
      ...
      for (const m of mappings) {
        const key =
          m.folder_id === null || m.folder_id === undefined
            ? "0"
            : m.folder_id.toString();
        if (!groups[key]) groups[key] = [];
        groups[key].push(m.message_id);
      }
      ...
      const targetPeerId =
        resolvedTargetPeer instanceof Api.User ||
        resolvedTargetPeer instanceof Api.Channel ||
        resolvedTargetPeer instanceof Api.Chat
          ? Number(resolvedTargetPeer.id)
          : 0;

      // Update local index immediately with new IDs and folder
      await moveFilesIndex(
        session.phone,
        allMappings,
        targetFolderId,
        targetPeerId,
  ...
  .post(
    "/rename",
    async ({ body: { folder_id, message_id, name }, sessionId }) => {
      const session = await getSession(sessionId!);
      if (!session) throw new ApiError(401, "Session not found");
      ...
      const fId =
        folder_id !== undefined && folder_id !== null
          ...
          : null;
      const mId =
        typeof message_id === "string" ? parseInt(message_id, 10) : message_id;

      await renameLocal(session.phone, fId ?? 0, mId, name);
      await renameTelegram(sessionId!, fId, mId, name);

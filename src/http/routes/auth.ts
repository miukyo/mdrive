import { Elysia, t } from "elysia";
import { ApiError } from "../errors.js";
import {
  getTelegramClient,
  logoutAndDestroySession,
  removeTelegramClient,
} from "../../services/telegram.js";
import {
  createOrUpdateSession,
  getSession,
  getActiveSessionByPhone,
  deleteOtherSessionsByPhone,
} from "../../repositories/sessions.js";
import { Api } from "telegram";
import { config } from "../../config.js";
import { refreshIndex } from "../../services/indexer.js";
import { hashPhone } from "../../db.js";

export const authRouter = new Elysia({ prefix: "/auth" })
  .post(
    "/send-code",
    async ({ body: { phone, force_sms, pin } }) => {
      if (!phone) {
        throw new ApiError(400, "phone is required");
      }

      if (!force_sms) {
        const existing = await getActiveSessionByPhone(phone);

        // If we have an active session with a PIN, handle quick login
        if (existing && existing.pin) {
          if (pin) {
            if (existing.pin === pin) {
              return { session_id: existing.id, status: "logged_in" };
            } else {
              throw new ApiError(403, "Invalid PIN");
            }
          }
          // PIN is required but not provided
          return { session_id: existing.id, status: "pin_required" };
        }

        // If no session exists or session has no PIN, we ignore the 'pin' argument
        // and proceed to send an OTP for a fresh login.
      }

      const api_id = config.TELEGRAM_API_ID;
      const api_hash = config.TELEGRAM_API_HASH;

      if (!api_id || !api_hash) {
        throw new ApiError(
          500,
          "Server is missing TELEGRAM_API_ID or TELEGRAM_API_HASH configuration",
        );
      }

      const sessionId = crypto.randomUUID();
      
      // Initialize the session in DB first, so getTelegramClient can find it
      await createOrUpdateSession({
        id: sessionId,
        phone,
        api_id,
        api_hash,
        auth_state: "pending_code",
      });

      const client = await getTelegramClient(sessionId);
      const { phoneCodeHash } = await client.sendCode(
        { apiId: api_id, apiHash: api_hash },
        phone,
      );

      // Update session with the received phone_code_hash
      await createOrUpdateSession({
        id: sessionId,
        phone_code_hash: phoneCodeHash,
      });

      return { session_id: sessionId, status: "code_sent" };
    },
    {
      body: t.Object({
        phone: t.String({
          description: "Phone number in international format",
        }),
        force_sms: t.Optional(t.Boolean({ default: false })),
        pin: t.Optional(
          t.String({ description: "Optional PIN for quick login" }),
        ),
      }),
      detail: {
        summary: "Send authentication code",
        tags: ["Auth"],
      },
    },
  )
  .post(
    "/sign-in",
    async ({ body: { session_id, code }, headers }) => {
      const sid = session_id || headers["x-session-id"];
      if (!sid) {
        throw new ApiError(400, "Missing session_id or x-session-id header");
      }

      const session = await getSession(sid);
      if (!session || !session.phone_code_hash) {
        throw new ApiError(400, "Invalid session or missing phone_code_hash");
      }

      const client = await getTelegramClient(sid);

      try {
        await client.invoke(
          new Api.auth.SignIn({
            phoneNumber: session.phone,
            phoneCodeHash: session.phone_code_hash,
            phoneCode: code,
          }),
        );

        const newSessionString = client.session.save() as unknown as string;
        const updates: any = {
          id: sid,
          phone: hashPhone(session.phone),
          auth_state: "logged_in",
          session_string: newSessionString,
        };

        await createOrUpdateSession(updates);
        const deletedIds = await deleteOtherSessionsByPhone(
          hashPhone(session.phone),
          sid,
        );
        for (const id of deletedIds) {
          await removeTelegramClient(id);
        }
        refreshIndex(sid).catch(console.error);
        return { status: "logged_in" };
      } catch (err: any) {
        if (err.message.includes("SESSION_PASSWORD_NEEDED")) {
          await createOrUpdateSession({
            id: sid,
            auth_state: "pending_password",
          });
          return { status: "password_required" };
        } else {
          throw new ApiError(500, err.message);
        }
      }
    },
    {
      body: t.Object({
        session_id: t.Optional(t.String()),
        code: t.String({
          description: "The verification code received via Telegram/SMS",
        }),
      }),
      headers: t.Object({
        "x-session-id": t.Optional(t.String()),
      }),
      detail: {
        summary: "Verify authentication code",
        tags: ["Auth"],
      },
    },
  )
  .post(
    "/login-2fa",
    async ({ body: { session_id, password } }) => {
      const client = await getTelegramClient(session_id);
      const session = await getSession(session_id);
      if (!session) throw new ApiError(400, "Session not found");

      await client.signInWithPassword(
        {
          apiId: session!.api_id,
          apiHash: session!.api_hash,
        },
        {
          password: async () => password,
          onError: (e: any) => {
            if (e.message.includes("PASSWORD_HASH_INVALID")) {
              throw new ApiError(403, "Invalid password");
            }
            throw new ApiError(500, e.message);
          },
        },
      );

      const newSessionString = client.session.save() as unknown as string;
      const updates: any = {
        id: session_id,
        phone: hashPhone(session.phone),
        auth_state: "logged_in",
        session_string: newSessionString,
      };

      await createOrUpdateSession(updates);
      const deletedIds = await deleteOtherSessionsByPhone(
        hashPhone(session.phone),
        session_id,
      );
      for (const id of deletedIds) {
        await removeTelegramClient(id);
      }

      refreshIndex(session_id).catch(console.error);

      return { status: "logged_in" };
    },
    {
      body: t.Object({
        session_id: t.String(),
        password: t.String({ description: "2FA Password" }),
      }),
      detail: {
        summary: "Login with 2FA password",
        tags: ["Auth"],
      },
    },
  )
  .post(
    "/set-pin",
    async ({ body: { pin }, headers }) => {
      const sessionId = headers["x-session-id"]!;
      await createOrUpdateSession({ id: sessionId, pin });
      return { status: "pin_updated" };
    },
    {
      headers: t.Object({
        "x-session-id": t.String(),
      }),
      body: t.Object({
        pin: t.String({ minLength: 4, description: "New quick-login PIN" }),
      }),
      detail: {
        summary: "Set quick-login PIN",
        tags: ["Auth"],
      },
    },
  )
  .post(
    "/logout",
    async ({ headers }) => {
      const sessionId = headers["x-session-id"]!;
      await logoutAndDestroySession(sessionId);
      return { status: "logged_out" };
    },
    {
      headers: t.Object({
        "x-session-id": t.String(),
      }),
      detail: {
        summary: "Logout and destroy session",
        tags: ["Auth"],
      },
    },
  )
  .get(
    "/me",
    async ({ headers }) => {
      const sessionId = headers["x-session-id"]!;
      const client = await getTelegramClient(sessionId);
      const me = await client.getMe();
      return { user: me };
    },
    {
      headers: t.Object({
        "x-session-id": t.String(),
      }),
      detail: {
        summary: "Get current user profile",
        tags: ["Auth"],
      },
    },
  )
  .get(
    "/avatar",
    async ({ headers, query }) => {
      const sessionId = (headers["x-session-id"] || query.session_id) as string;
      if (!sessionId) return new Response(null, { status: 401 });
      
      const client = await getTelegramClient(sessionId);
      const me = await client.getMe();

      if (me instanceof Api.User && me.photo) {
        const buffer = await client.downloadProfilePhoto(me);
        if (buffer) {
          return new Response(buffer, {
            headers: {
              "Content-Type": "image/jpeg",
              "Cache-Control": "public, max-age=3600",
            },
          });
        }
      }
      return new Response(null, { status: 404 });
    },
    {
      headers: t.Object({
        "x-session-id": t.Optional(t.String()),
      }),
      query: t.Object({
        session_id: t.Optional(t.String()),
      }),
      detail: {
        summary: "Get current user profile photo",
        tags: ["Auth"],
      },
    },
  );

...
import { getTelegramClient, logoutAndDestroySession, removeTelegramClient } from '../../services/telegram.js';
import { createOrUpdateSession, getSession, getActiveSessionByPhone, deleteOtherSessionsByPhone } from '../../repositories/sessions.js';
...
authRouter.post('/send-code', async (req, res, next) => {
  try {
    ...
    if (!api_id || !api_hash) {
      throw new ApiError(500, 'Server is missing TELEGRAM_API_ID or TELEGRAM_API_HASH configuration');
    }
...
authRouter.post('/sign-in', async (req, res, next) => {
  try {
    ...
    try {
      ...
      const updates: any = { id: sessionId, auth_state: 'logged_in', session_string: newSessionString };

      await createOrUpdateSession(updates);
      const deletedIds = await deleteOtherSessionsByPhone(session.phone, sessionId);
      ...
      if (error.message.includes('SESSION_PASSWORD_NEEDED')) {
        await createOrUpdateSession({ id: sessionId, auth_state: 'pending_password' });
